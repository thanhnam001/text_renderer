...
OUT_DIR = CURRENT_DIR / "output_sim"
DATA_DIR = CURRENT_DIR
BG_DIR = DATA_DIR / "card_bg"
...
def get_char_corpus():
    return CharCorpus(
        CharCorpusCfg(
            text_paths=[TEXT_DIR / "ptg_text.txt"],
            filter_by_chars=True,
            chars_file=CHAR_DIR / "ptg.txt",
            length=(5, 10),
...
def enum_data1():
    # paths=[TEXT_DIR/'synthetic'/ file for file in os.listdir(TEXT_DIR/'synthetic')]
    return base_cfg(
        inspect.currentframe().f_code.co_name,
        corpus=EnumCorpus(
            EnumCorpusCfg(
                text_paths=[TEXT_DIR / "sim_v2.txt"],
                filter_by_chars=True,
...
def enum_data2():
    ...
    return base_cfg(
        ...
        corpus=EnumCorpus(
            EnumCorpusCfg(
                text_paths=[TEXT_DIR / "card_v2.txt"],
                filter_by_chars=True,
                chars_file=CHAR_DIR / "num.txt",
                **font_cfg
            ),
...
def rand_data():
    return base_cfg(
        inspect.currentframe().f_code.co_name,
        corpus=RandCorpus(
            RandCorpusCfg(chars_file=CHAR_DIR / "ptg.txt", **font_cfg),
        ),
    )
...
def same_line_data():
    return base_cfg(
        ...
        corpus=[
            EnumCorpus(
                EnumCorpusCfg(
                    text_paths=[TEXT_DIR / "all_names_unique.txt"],
                    filter_by_chars=True,
                    chars_file=CHAR_DIR / "ptg.txt",
                    **font_cfg
                ),
